...
if (hasInterface) then
{
	...
	_veh addAction ["<img image='client\icons\store.paa'/> Repaint vehicle", "addons\VehiclePainter\VehiclePainter_Check.sqf", [], 50, false, true, "", "alive _target && alive objectParent _this && attachedTo _target != vehicle _this && _target distance vehicle _this <= (10 max (sizeOf typeOf vehicle _this * 0.75)) && (isNil 'mutexScriptInProgress' || {!mutexScriptInProgress})"];
};
